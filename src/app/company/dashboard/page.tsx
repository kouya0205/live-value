...
import { mdiAccountTie, mdiEmailOutline, mdiEyeOffOutline, mdiFileOutline, mdiLogout, mdiMagnify, mdiMessageReplyOutline, mdiStarOutline } from '@mdi/js';
  ...
  return (
      ...
      <main className="flex p-4">
        ...
        <Tabs defaultValue="all" className="mb-4">
          <TabsList className="flex space-x-4">
                ...
                {value: 'hidden', label: '非表示人材'}].map((tab, index) => (
              <TabsTrigger key={index} value={tab.value} className="rounded-full data-[state=active]:bg-gradient-to-r data-[state=active]:from-blue-500 data-[state=active]:to-cyan-400 data-[state=active]:text-white bg-gray-200 px-6 py-2">
                {tab.label}
              </TabsTrigger>
            ))}
          </TabsList>
    ...
    <div className="space-y-6">
      {articles.data.contents.map((article) => (
        <Card key={article.id} className="p-6 shadow-lg hover:shadow-xl transition-shadow duration-300">
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
              ...
              <div className="flex flex-col lg:flex-row gap-6">
                ...
                <Card className="p-4 h-full">
                  ...
                  <div className="bg-gray-200 h-5/6 rounded-lg" aria-label="スキルテスト結果のグラフ" />
                </Card>
                </div>

              <div className="w-full lg:w-1/2">
                <Link href={article.noteUrl} className="block">
      ...
      <div className='bg-gray-100 mt-10 pr-5'>
        <SearchFilters />
      </div>
    </div>
  );
